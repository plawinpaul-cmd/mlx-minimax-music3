...
class MiniMaxMusic3Pipeline:
    ...
    def generate_frame_hiddens(
        ...
        for frame_index in range(max_frames + 1):
            ...
            mx.eval(sampled)
            if sampled.item() == self.model_config.audio_end_token_id:
                break

            semantic_code = (sampled - self.model_config.audio_code_offset).astype(mx.int32)
    ...
    def denoise_chunks(
        ...
        for start in starts:
            ...
            for timestep in timesteps:
                ...
                conditional = self.components.transformer(latents, time_batch, condition)
                unconditional = self.components.transformer(
                    latents,
                    time_batch,
