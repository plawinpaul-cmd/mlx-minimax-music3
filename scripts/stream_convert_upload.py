...
def remote_files(api: HfApi, repo_id: str) -> tuple[str, dict[str, object]]:
    info = api.model_info(repo_id, files_metadata=True)
    return info.sha, {sibling.rfilename: sibling for sibling in info.siblings}
...
def verify_and_mark(
    ...
    for shard in manifest["shards"]:
        ...
        if remote_hash != shard["sha256"]:
            ...
            pending_uploads.append(shard["file"])

    if pending_uploads:
        api.upload_folder(
            repo_id=repo_id,
            repo_type="model",
            folder_path=component_dir,
            path_in_repo=component,
            allow_patterns=pending_uploads,
            commit_message=f"upload converted {component} shards",
        )
        revision, remote_by_path = remote_files(api, repo_id)
    ...
    if manifest["status"] == "complete":
        metadata.append("model.safetensors.index.json")
    api.upload_folder(
        repo_id=repo_id,
        repo_type="model",
        folder_path=component_dir,
